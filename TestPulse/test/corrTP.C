...
void corrTP()
{
  ...
  fRatioInput.open("TP_Gainratio-00045210_DAC0.txt");
  ...
  for(Int_t iXtal = 0; !fRatioInput.eof(); iXtal++) {
    Int_t Xtal, eta, phi;
    fRatioInput >>  Xtal >> eta >> phi >> ratio121New[iXtal] >> ratio126New[iXtal];
    if(Xtal != iXtal) cout << " pb Xtal " << Xtal << " " << iXtal << endl;
  ...
  // Open old ratio file
  ifstream fRatioInput;
  fRatioInput.open("TP_Gainratio-00045210_DAC2.txt");
  if(!fRatioInput.is_open()) {
  ...
  for(Int_t iXtal = 0; !fRatioInput.eof(); iXtal++) {
    Int_t Xtal;
    fRatioInput >>  Xtal >> eta >> phi >> ratio121Old[iXtal] >> ratio126Old[iXtal];
    if(Xtal != iXtal) cout << " pb Xtal " << Xtal << " " << iXtal << endl;
  ...
  legend1->AddEntry(hRatio126New, "3 pC","f");
  hRatio126New->Write();
  ...
  legend1->AddEntry(hRatio126Old, "4.2 pC","f");
  legend1->Draw();
  cRatio->Modified();
  hRatio126Old->Write();
  ...
  hCorr126->Draw();
  hCorr126->GetXaxis()->SetTitle("3 pC");
  hCorr126->GetYaxis()->SetTitle("4.2 pC");
